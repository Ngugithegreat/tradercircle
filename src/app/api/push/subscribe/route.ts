...
export async function POST(req: Request) {
  ...
  const sql = db();
  await sql`
    INSERT INTO tradercircle_push_subs (endpoint, user_id, p256dh, auth)
...
export async function DELETE(req: Request) {
  ...
  const sql = db();
  if (endpoint) {
    await sql`DELETE FROM tradercircle_push_subs WHERE endpoint = ${endpoint} AND user_id = ${session.id}`;
  }
  return NextResponse.json({ ok: true });
}
