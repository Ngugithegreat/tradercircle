...
export async function POST(req: Request) {
  ...
  const flow = (await sql`
    SELECT
      ...
      COALESCE((SELECT COUNT(*) FROM tradercircle_trades
                 WHERE user_id = ${session.id} AND is_demo = false), 0) AS trades
    FROM tradercircle_users WHERE id = ${session.id} LIMIT 1
  ...
  const debit = (await sql`
    UPDATE tradercircle_users SET balance = balance - ${amount}
  ...
  if (await isWithdrawBlocked(session.id, flow[0]?.email)) {
    const rows = (await sql`
      INSERT INTO tradercircle_transactions (user_id, type, amount, status, method, reference, note)
  ...
  if (automated && phone && isTeronaConfigured() && !preferB2cForMpesa) {
    ...
    if (!tp.ok) {
      ...
      await sql`UPDATE tradercircle_users SET balance = balance + ${amount} WHERE id = ${session.id}`;
      return NextResponse.json(
    ...
    const rows = (await sql`
      INSERT INTO tradercircle_transactions
