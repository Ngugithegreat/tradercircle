...
export async function GET() {
  ...
  const [userRows, txns, openTrades, closedTrades, refStats] = await Promise.all([
    sql`SELECT id, name, email, role, balance, demo_balance, country, phone, status, kyc_status, kyc_reason, bonus_locked, is_test, test_win_pct, email_verified FROM tradercircle_users WHERE id = ${session.id}` as Promise<any[]>,
    sql`SELECT * FROM tradercircle_transactions WHERE user_id = ${session.id} ORDER BY created_at DESC LIMIT 40` as Promise<any[]>,
    sql`SELECT * FROM tradercircle_trades WHERE user_id = ${session.id} AND status = 'open' ORDER BY created_at DESC` as Promise<any[]>,
    sql`SELECT * FROM tradercircle_trades WHERE user_id = ${session.id} AND status != 'open' ORDER BY created_at DESC LIMIT 40` as Promise<any[]>,
