...
export async function GET() {
  ...
  const [pending, users, kpi, daily, topUsers, kycPending, testAccounts, withdrawalsRaw, attentionRaw] = await Promise.all([
    sql`
      ...
      FROM tradercircle_transactions t JOIN tradercircle_users u ON u.id = t.user_id
      WHERE t.status = 'pending'
      ORDER BY t.created_at ASC
      LIMIT 300
    ` as Promise<any[]>,
    sql`SELECT id, name, email, role, balance, created_at FROM tradercircle_users ORDER BY created_at DESC LIMIT 100` as Promise<any[]>,
    sql`
      SELECT
        (SELECT COUNT(*) FROM tradercircle_users) AS user_count,
        (SELECT COALESCE(SUM(balance),0) FROM tradercircle_users) AS total_balance,
        (SELECT COUNT(*) FROM tradercircle_trades WHERE is_demo = false) AS trade_count,
        (SELECT COUNT(*) FROM tradercircle_trades WHERE status = 'won' AND is_demo = false) AS won_count,
        (SELECT COUNT(*) FROM tradercircle_trades WHERE status = 'lost' AND is_demo = false) AS lost_count,
        (SELECT COALESCE(SUM(amount),0) FROM tradercircle_transactions WHERE type='deposit' AND status='completed') AS deposits_total,
        (SELECT COALESCE(SUM(-amount),0) FROM tradercircle_transactions WHERE type='withdrawal' AND status='completed') AS withdrawals_total,
        (SELECT COUNT(*) FROM tradercircle_transactions WHERE type='deposit' AND status='pending') AS deposits_pending,
        (SELECT COUNT(*) FROM tradercircle_transactions WHERE type='withdrawal' AND status='pending') AS withdrawals_pending,
        (SELECT COALESCE(SUM(-amount),0) FROM tradercircle_transactions WHERE type='trade_stake' AND is_demo = false) AS staked_total,
        (SELECT COALESCE(SUM(amount),0) FROM tradercircle_transactions WHERE type='trade_payout' AND is_demo = false) AS payout_total,
        (SELECT COALESCE(SUM(amount),0) FROM tradercircle_transactions WHERE type='bonus' AND amount > 0) AS bonus_issued,
        (SELECT COALESCE(SUM(bonus_locked),0) FROM tradercircle_users) AS bonus_locked
    ` as Promise<any[]>,
    sql`
      SELECT to_char(date_trunc('day', created_at), 'YYYY-MM-DD') AS day,
             COALESCE(SUM(-amount),0) AS volume
      FROM tradercircle_transactions
    ...
    sql`
      WITH recent AS (
        ...
        FROM tradercircle_users ORDER BY created_at DESC LIMIT 100
      )
      SELECT u.id, u.name, u.email, u.balance, u.status, u.promo, u.withdraw_blocked, u.created_at,
        ...
        COALESCE((SELECT SUM(x.amount) FROM tradercircle_transactions x
        ...
        (SELECT x.method FROM tradercircle_transactions x
      ...
      FROM recent u
      LEFT JOIN tradercircle_trades t ON t.user_id = u.id AND t.is_demo = false
    ...
    sql`
      ...
      FROM tradercircle_users
      WHERE kyc_status = 'pending'
      ORDER BY kyc_submitted_at ASC NULLS LAST
      LIMIT 1000
    ` as Promise<any[]>,
    sql`SELECT id, name, email, test_win_pct FROM tradercircle_users WHERE is_test = true ORDER BY email` as Promise<any[]>,
    sql`
