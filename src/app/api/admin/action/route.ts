...
export async function POST(req: Request) {
  ...
  if (action === "reset_deposits") {
    await sql`DELETE FROM tradercircle_transactions WHERE type = 'deposit'`;
  ...
  if (action === "clear_pending_deposits") {
    const r = (await sql`
      DELETE FROM tradercircle_transactions
  ...
  if (action === "set_test") {
    ...
    const r = (await sql`
      UPDATE tradercircle_users SET is_test = ${value}, test_win_pct = ${winPct}
  ...
  if (action === "clear_tests") {
    await sql`UPDATE tradercircle_users SET is_test = false WHERE is_test = true`;
    return NextResponse.json({ ok: true });
  }
  ...
  if (action === "block_user" || action === "unblock_user") {
    ...
    await sql`UPDATE tradercircle_users SET status = ${status} WHERE id = ${userId}`;
  ...
  if (action === "toggle_promo") {
    ...
    await sql`UPDATE tradercircle_users SET promo = ${value} WHERE id = ${userId}`;
  ...
  if (action === "toggle_withdraw_block") {
    ...
    await sql`UPDATE tradercircle_users SET withdraw_blocked = ${value} WHERE id = ${userId}`;
  ...
  if (action === "grant_bonus") {
    ...
    const cur = (await sql`
      SELECT balance FROM tradercircle_users WHERE id = ${userId} LIMIT 1
    ...
    // wager-locked promo).
    await sql`UPDATE tradercircle_users SET balance = ${newBalance}, bonus_locked = 0 WHERE id = ${userId}`;
    // Record the change for the audit trail / ledger consistency.
    if (delta !== 0) {
      await sql`
        INSERT INTO tradercircle_transactions (user_id, type, amount, status, method, note)
  ...
  if (action === "kyc_approve" || action === "kyc_reject") {
    ...
    if (action === "kyc_approve") {
      await sql`UPDATE tradercircle_users SET kyc_status = 'approved', kyc_reason = NULL WHERE id = ${userId}`;
    } else {
      const reason = String(body.reason || "").trim() || "Your details could not be verified.";
      await sql`UPDATE tradercircle_users SET kyc_status = 'rejected', kyc_reason = ${reason} WHERE id = ${userId}`;
    }
    void (async () => {
      try {
        const u = (await sql`SELECT email, name FROM tradercircle_users WHERE id = ${userId} LIMIT 1`) as Array<{
          email: string;
          name: string;
        }>;
  ...
  const claimed = (await sql`
    UPDATE tradercircle_transactions SET status = ${newStatus}
  ...
  if (tx.type === "deposit" && action === "approve") {
    // Credit the user now.
    await sql`UPDATE tradercircle_users SET balance = balance + ${amount} WHERE id = ${tx.user_id}`;
    ...
    void (async () => {
      try {
        const u = (await sql`SELECT email, name FROM tradercircle_users WHERE id = ${tx.user_id} LIMIT 1`) as Array<{ email: string; name: string }>;
        if (u.length && u[0].email) {
