...
export async function POST(req: Request) {
  ...
  if (demo && (kind === "rise_fall" || kind === "digit" || kind === "mult")) {
    ...
    if (globalTest || body.testMode) {
      ...
      const ur = (await sql`SELECT email, is_test, test_win_pct FROM tradercircle_users WHERE id = ${session.id} LIMIT 1`) as Array<{
        email: string;
        is_test: boolean;
        test_win_pct: number;
      }>;
  ...
  const debit = (
    demo
      ? await sql`
          UPDATE tradercircle_users SET demo_balance = demo_balance - ${stake}
          WHERE id = ${session.id} AND demo_balance >= ${stake}
          RETURNING demo_balance AS balance
        `
      : await sql`
          UPDATE tradercircle_users SET balance = balance - ${stake}
  ...
  if (kind === "rise_fall") {
    ...
    rows = (await sql`
      INSERT INTO tradercircle_trades
