...
export async function POST(req: Request) {
  ...
  const rows = (await sql`
    SELECT * FROM tradercircle_trades WHERE id = ${id} AND user_id = ${session.id} LIMIT 1
  `) as TradeRow[];
  ...
  try {
    ...
    const bal = (await sql`SELECT balance, demo_balance FROM tradercircle_users WHERE id = ${session.id}`) as any[];
