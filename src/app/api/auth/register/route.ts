...
export async function POST(req: Request) {
  try {
    ...
    const existing = (await sql`SELECT id FROM tradercircle_users WHERE email = ${cleanEmail} LIMIT 1`) as any[];
    if (existing.length) {
      return NextResponse.json(
    ...
    const anyUser = (await sql`SELECT 1 FROM tradercircle_users LIMIT 1`) as any[];
    ...
    if (ref) {
      ...
      if (rid) {
        const r = (await sql`SELECT id FROM tradercircle_users WHERE id = ${rid} LIMIT 1`) as any[];
        if (r.length) referredBy = rid;
      }
    }

    const rows = (await sql`
      INSERT INTO tradercircle_users (name, email, password_hash, role, balance, country, phone, referred_by)
