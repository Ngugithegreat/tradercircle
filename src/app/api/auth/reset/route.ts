...
export async function POST(req: Request) {
  ...
  const rows = (await sql`
    SELECT id, user_id FROM tradercircle_password_resets
  ...
  await sql`UPDATE tradercircle_users SET password_hash = ${hash} WHERE id = ${user_id}`;
  // Single-use: burn this token and any other outstanding ones for the user.
  await sql`UPDATE tradercircle_password_resets SET used = true WHERE user_id = ${user_id}`;
  void id;
