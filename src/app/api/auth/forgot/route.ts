...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rows = (await sql`
      SELECT id, name, email FROM tradercircle_users WHERE email = ${clean} LIMIT 1
    ...
    await sql`UPDATE tradercircle_password_resets SET used = true WHERE user_id = ${user.id} AND used = false`;
    await sql`
      INSERT INTO tradercircle_password_resets (user_id, token_hash, expires_at)
