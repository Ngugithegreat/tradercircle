...
export async function POST(req: Request) {
  ...
  const found = (await sql`
    SELECT * FROM tradercircle_transactions
  ...
  if (!verified || !callbackSuccess) {
    ...
    await sql`
      UPDATE tradercircle_transactions SET status = 'rejected', note = ${
  ...
  if (claimed.length) {
    await sql`UPDATE tradercircle_users SET balance = balance + ${Number(tx.amount)} WHERE id = ${tx.user_id}`;
  }
