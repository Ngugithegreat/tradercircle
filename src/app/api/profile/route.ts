...
export async function GET() {
  ...
  const rows = (await sql`
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM tradercircle_transactions
      ...
      COALESCE((SELECT COUNT(*) FROM tradercircle_trades WHERE user_id = u.id AND status<>'open' AND is_demo=false),0) AS trades,
      COALESCE((SELECT COUNT(*) FROM tradercircle_trades WHERE user_id = u.id AND status='won' AND is_demo=false),0) AS wins,
      COALESCE((SELECT SUM(CASE WHEN status='won' THEN payout - stake WHEN status='lost' THEN -stake ELSE 0 END)
                 FROM tradercircle_trades WHERE user_id = u.id AND status<>'open' AND is_demo=false),0) AS pnl
    FROM tradercircle_users u WHERE u.id = ${session.id} LIMIT 1
  `) as any[];
