...
export async function POST(req: Request) {
  ...
  const me = (await sql`SELECT phone FROM tradercircle_users WHERE id = ${session.id} LIMIT 1`) as Array<{ phone: string | null }>;
  ...
  const taken = (await sql`
    SELECT 1 FROM tradercircle_users WHERE phone = ${newPhone} AND id <> ${session.id} LIMIT 1
  `) as any[];
  if (taken.length) {
  ...
  if (!isSmsConfigured()) {
    await sql`UPDATE tradercircle_users SET phone = ${newPhone} WHERE id = ${session.id}`;
