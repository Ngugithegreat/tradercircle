...
const COOKIE = "tradercircle_session";
const ADMIN_COOKIE = "tradercircle_admin";
const DAY = 60 * 60 * 24;
...
export async function currentUser(): Promise<{
  ...
  const rows = (await sql`
    SELECT id, email, name, role, balance, country FROM tradercircle_users WHERE id = ${s.id} LIMIT 1
  `) as Array<{
    id: number;
    email: string;
    name: string;
