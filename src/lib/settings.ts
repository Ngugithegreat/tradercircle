...
// Runtime, admin-tunable settings kept in the tradercircle_settings key/value table.
...
async function rawSetting(key: string): Promise<string | null> {
  ...
  await ensureSchema();
  const sql = db();
  const rows = (await sql`SELECT value FROM tradercircle_settings WHERE key = ${key} LIMIT 1`) as Array<{ value: string }>;
...
export async function isBlocked(userId: number): Promise<boolean> {
  ...
  const rows = (await sql`
    SELECT status FROM tradercircle_users WHERE id = ${userId} LIMIT 1
...
export async function isWithdrawBlocked(
  ...
  const sql = db();
  const rows = (await sql`
    SELECT COALESCE(withdraw_blocked, false) AS b FROM tradercircle_users WHERE id = ${userId} LIMIT 1
  `) as Array<{ b: boolean }>;
