...
export async function payReferralOnDeposit(
  ...
  const claimed = (await sql`
    UPDATE tradercircle_users SET referral_rewarded = true
  ...
  await sql`UPDATE tradercircle_users SET balance = balance + ${reward} WHERE id = ${referrerId}`;
  await sql`
  ...
  try {
    const r = (await sql`SELECT email, name FROM tradercircle_users WHERE id = ${referrerId} LIMIT 1`) as Array<{
      email: string;
      name: string;
    }>;
...
export async function referralStats(
  ...
  const rows = (await sql`
    SELECT
      (SELECT COUNT(*) FROM tradercircle_users WHERE referred_by = ${userId}) AS referred_count,
      (SELECT COALESCE(SUM(amount), 0) FROM tradercircle_transactions
