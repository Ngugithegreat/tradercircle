...
// Signup phone-OTP. Codes are stored HASHED in tradercircle_otps (never plaintext),
...
export async function sendSignupOtp(
  ...
  const existing = (await sql`
    SELECT last_sent_at FROM tradercircle_otps WHERE phone = ${phone} LIMIT 1
  ...
  await sql`
    INSERT INTO tradercircle_otps (phone, code_hash, expires_at, attempts, last_sent_at)
...
export async function verifySignupOtp(
  ...
  const rows = (await sql`
    SELECT code_hash, expires_at, attempts FROM tradercircle_otps WHERE phone = ${phone} LIMIT 1
  ...
  if (hashCode(phone, clean) !== otp.code_hash) {
    await sql`UPDATE tradercircle_otps SET attempts = attempts + 1 WHERE phone = ${phone}`;
    return { ok: false, error: "Incorrect code. Try again." };
  }

  // Correct — consume it so it can't be reused.
  await sql`DELETE FROM tradercircle_otps WHERE phone = ${phone}`;
  return { ok: true };
}
