...
export async function settleTrade(trade: TradeRow): Promise<TradeRow> {
  ...
  if (!updated.length) {
    const latest = (await sql`SELECT * FROM tradercircle_trades WHERE id = ${trade.id}`) as TradeRow[];
  ...
  if (won) {
    ...
    if (trade.is_demo) {
      await sql`UPDATE tradercircle_users SET demo_balance = demo_balance + ${payout} WHERE id = ${trade.user_id}`;
    } else {
      await sql`UPDATE tradercircle_users SET balance = balance + ${payout} WHERE id = ${trade.user_id}`;
    }
    await sql`
      INSERT INTO tradercircle_transactions (user_id, type, amount, status, method, note, is_demo)
...
export async function settleExpiredTrades(userId: number): Promise<void> {
  ...
  const open = (await sql`
    SELECT * FROM tradercircle_trades
