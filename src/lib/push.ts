...
export async function sendPushToUser(userId: number, payload: PushPayload): Promise<void> {
  ...
  try {
    ...
    const subs = (await sql`
      SELECT endpoint, p256dh, auth FROM tradercircle_push_subs WHERE user_id = ${userId}
      ...
      subs.map(async (s) => {
        try {
          ...
          if (e?.statusCode === 404 || e?.statusCode === 410) {
            await sql`DELETE FROM tradercircle_push_subs WHERE endpoint = ${s.endpoint}`.catch(() => {});
          }
        }
      })
    );
