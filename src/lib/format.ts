...
/** Public-facing account number derived from the user id, e.g. TC-100482. */
export function accountNo(id: number): string {
  return "TC-" + String(100000 + Number(id));
}

/** Parse an account number / referral code (TC-100482 or ST100482) back to a user id. */
export function idFromAccountNo(code: string): number | null {
  const m = String(code).trim().toUpperCase().match(/^TC-?(\d{5,})$/);
  if (!m) return null;
