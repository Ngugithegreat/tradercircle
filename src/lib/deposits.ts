...
export async function creditPendingDeposit(
  ...
  const rows = (await sql`
    SELECT id, user_id, amount, status FROM tradercircle_transactions
  ...
  const claimed = (await sql`
    UPDATE tradercircle_transactions
  ...
  void (async () => {
    try {
      const u = (await sql`
        ...
        FROM tradercircle_users u JOIN tradercircle_transactions t ON t.id = ${tx.id}
...
export async function reconcilePendingMpesaDeposits(userId: number): Promise<void> {
  ...
  const pending = (await sql`
    SELECT provider_ref FROM tradercircle_transactions
...
export async function reconcilePendingCryptoDeposits(userId: number): Promise<void> {
  ...
  const sql = db();
  const pending = (await sql`
    SELECT provider_ref, receipt FROM tradercircle_transactions
    WHERE user_id = ${userId}
...
export async function reconcilePendingTeronaPayouts(userId: number): Promise<void> {
  ...
  const sql = db();
  const pending = (await sql`
    SELECT id, provider_ref, amount, method FROM tradercircle_transactions
  ...
  for (const w of pending) {
    try {
      ...
      if (isPaid(tp.data.status)) {
        await sql`UPDATE tradercircle_transactions SET status = 'completed', note = 'Payout completed' WHERE id = ${w.id} AND status = 'pending'`;
      } else if (isFailed(tp.data.status)) {
        const r = (await sql`
          UPDATE tradercircle_transactions SET status = 'rejected', note = 'Payout failed — refunded'
        ...
        if (r.length) {
          ...
          await sql`UPDATE tradercircle_users SET balance = balance + ${refund} WHERE id = ${r[0].user_id}`;
        }
      }
    } catch {
      /* best-effort */
...
export async function reconcilePendingTeronaDeposits(userId: number): Promise<void> {
  ...
  const sql = db();
  const pending = (await sql`
    SELECT provider_ref, method FROM tradercircle_transactions
...
export async function reconcileAllPendingTeronaPayouts(
  ...
  const sql = db();
  const pending = (await sql`
    SELECT id, user_id, provider_ref, amount, method FROM tradercircle_transactions
  ...
  for (const w of pending) {
    try {
      ...
      if (isPaid(tp.data.status)) {
        const r = (await sql`
          UPDATE tradercircle_transactions SET status = 'completed', note = 'Payout completed'
